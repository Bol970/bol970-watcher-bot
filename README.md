...
- ищет эфиры по названию, категории и имени преподавателя;
- открывает основные функции через кнопочное меню `/start` или `/menu`;
- показывает длинные списки по пять позиций с кнопками «Назад» и «Дальше»;
- для активного эфира открывает страницу в live-режиме и подсказывает нажать штатную кнопку запуска плеера;
...
- `/start`, `/help`, `/menu` — справка и кнопочное меню;
- `/watch` — пошаговая подписка;
- `/lessons [тема]` — три блока по пять уроков: сейчас, потом и далее;
- `/schedule [тема]` — полное расписание эфиров;
- `/teacher [имя]` — текущие и будущие эфиры преподавателя;
...
2. Отправить `/start`.
3. Проверить кнопки «Эфиры», «Всё расписание», «Преподаватель», «Фильмы» и «Новинки».
4. Отправить `/test` — бот сразу проверит LiveClasses, расписание/новинки LostFilm и каталог фильмов.
5. Написать `Следи за уроками по программированию`.
6. Запросить `/teacher Шадрин`.
7. Написать `Следи за сериалом Медведь`.
8. Запросить `Покажи новинки жанра драма`.
9. Запросить `Покажи фильмы жанра фантастика`.
